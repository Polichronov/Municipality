...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GestureDetector(
        onTap: () {
          // Hide keyboard and search results when tapping outside
        ...
        child: Column(
          children: [
            ...
            // Показваме или секциите, или резултатите от търсенето
            Expanded(
              flex: 2,
              child: _isSearching
  ...
  Widget _buildSearchSuggestions() {
    ...
    final List<String> filteredSuggestions = searchQuery.isEmpty
        ? _searchSuggestions
        : _searchSuggestions
            .where((suggestion) => suggestion.toLowerCase().contains(searchQuery))
            .toList();
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      return ListTile(
                        ...
                        leading: const Icon(Icons.search, color: Color(0xFF006C35)),
                        onTap: () {
                          _searchController.text = filteredSuggestions[index];
                          _performSearch(filteredSuggestions[index]);
                          setState(() {
                            _isSearching = false;
                          });
                          FocusScope.of(context).unfocus();
